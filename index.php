...
$recources = elgg_get_entities_from_metadata($params);
$go = false;
foreach($recources as $recource) {
	if ($go) {
		if ($category <> $recource->universal_categories) {
			$content_recources .= "<div class='clearfix'>"."</div><div class='c_i_c_bar_category'>" . $recource->universal_categories . "</div>";
			$content_recources .= "<div class='clearfix'><div class='c_i_c_bar_category>";
		}
	
	}
	if (!$go) {
		$category = $recource->universal_categories;
		$content_recources .= "<div class='frontpage_campaign-bar' clearfix>" . "<div class='c_i_c_bar'>" . elgg_echo('frontpage_campaign:recources') ."</div>" ;
		$content_recources .=   "<div class='c_i_c_bar_category'>" . $recource->universal_categories . "</div>" ;
	}
...
if (elgg_get_plugin_setting('numberofshownvideos', 'frontpage_campaign')){
	$numberofshownvideos= elgg_get_plugin_setting('numberofshownvideos', 'frontpage_campaign');
}
else {
	$limit=3;
}
...
$content_recources = "<div class='frontpage_campaign-bar'><div class='clearfix'>" . $content_recources ."&nbsp;" . "</div></div>"   ;
}


if ((int) $videolistcount > 0 ) {
	$content_videos = "<div class='frontpage_campaign-bar'><div class='clearfix'>" . "<div class='c_i_c_bar'>" . elgg_echo('frontpage_campaign:videos') . "</div><div class='frontpage_campaign-wrapper'>" .$content_videos ."&nbsp;" . $more_videos . "</div></div>"  . "</div>" ;
}
